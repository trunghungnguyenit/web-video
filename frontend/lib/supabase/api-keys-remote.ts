...
import { API_KEY_IDS } from '@/lib/api-keys-store';
...
export async function fetchRemoteApiKeys(
  ...
  const { data, error } = await supabase
    .from('user_api_keys')
    .select('gemini_key, veo_key, elevenlabs_key')
    .eq('user_id', userId)
    .maybeSingle();
